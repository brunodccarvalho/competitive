...
auto min_plus_concave_one(const vector<V>& a, const vector<V>& b) {
    ...
    auto better = [&](int i, int j, int t) { return cost(i, t) <= cost(j, t); };

    auto improv_prefix = [&](int u, int v, int l, int r) {
        while (r - l > 1) {
            int m = (l + r) / 2;
            better(u, v, m) ? r = m : l = m;
        }
        return l;
    };

    auto improv_suffix = [&](int u, int v, int l, int r) {
        while (r - l > 1) {
            int m = (l + r) / 2;
            better(u, v, m) ? l = m : r = m;
        }
        return r;
    };
    ...
    auto solve_prefix = [&](int L, int R) {
        for (int i = L, k = L, S = -1; k < R; i++, k++) {
            ...
            if (i < N) {
                int t = S < 0 ? R - 1 : improv_prefix(stk[S][0], i, k - 1, stk[S][1]);
                if (t >= k) {
                    stk.push_back({i, t}), S++;
                }
            }
    ...
    auto solve_suffix = [&](int L, int R) {
        for (int k = R - 1, i = k - M + 1, S = -1; k >= L; i--, k--) {
            while (S >= 0 && i >= 0 && better(i, stk[S][0], stk[S][1])) {
                stk.pop_back(), S--;
            }

            if (i >= 0) {
                int t = S < 0 ? L : improv_suffix(stk[S][0], i, stk[S][1], k + 1);
                if (t <= k) {
                    stk.push_back({i, t}), S++;
                }
            }
...
template <typename V>
auto min_plus_convex_one(const vector<V>& a, const vector<V>& b) {
    const int N = a.size();
    const int M = b.size();
    if (N == 0 || M == 0) {
        return N ? a : b;
    ...
    auto solve_prefix = [&](int L, int R) {
        for (int i = L, k = L, S = -1, H = 0; k < R; i++, k++) {
            while (S >= H && i < N && better(i, deq[S][0], deq[S][1])) {
                deq.pop_back(), S--;
            }

            if (i < N) {
                int t = S < H ? k : improv_prefix(deq[S][0], i, deq[S][1], R);
                if (t < R) {
                    deq.push_back({i, t}), S++;
                }
            }

            c[k] = min(c[k], cost(deq[H][0], k));

            if (S >= H) {
                ++deq[H][1];
                H += H < S && deq[H][1] == deq[H + 1][1];
            }
        }
        deq.clear();
    };
    ...
    auto solve_suffix = [&](int L, int R) {
        for (int k = R - 1, i = k - M + 1, S = -1, H = 0; k >= L; i--, k--) {
            while (S >= H && i >= 0 && better(i, deq[S][0], deq[S][1])) {
                deq.pop_back(), S--;
            }

            if (i >= 0) {
                int t = S < H ? k : improv_suffix(deq[S][0], i, L - 1, deq[S][1]);
                if (t >= L) {
                    deq.push_back({i, t}), S++;
                }
            }
...
template <typename V>
auto max_plus_concave_one(const vector<V>& a, const vector<V>& b) {
    const int N = a.size();
    const int M = b.size();
    if (N == 0 || M == 0) {
        return N ? a : b;
    ...
    const int C = N + M - 1;
    vector<V> c(C, numeric_limits<V>::min());
    vector<array<int, 2>> deq;
